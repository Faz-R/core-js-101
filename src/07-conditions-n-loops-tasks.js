...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const one = (a.x <= b.x && b.x <= a.x1);
  const two = (b.x <= a.x && a.x <= b.x1);
  const three = (a.y <= b.y && b.y <= a.y1);
  const four = (b.y <= a.y && a.y <= b.y1);
...
function isInsideCircle(circle, point) {
  const cir = circle.center;
  return (((cir.x - point.x) ** 2 + (cir.y - point.y) ** 2) < circle.radius ** 2);
}
